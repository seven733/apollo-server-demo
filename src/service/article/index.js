...
const { getPageInfo } = require('../../utils/page');
...
async function getArticles(query = {}) {
  ...
  const articles = await Article.find(query).sort({ num: -1, updatedAt: -1 }).skip(pageInfo.skip).limit(pageInfo.limit).lean();
...
async function getTagStatistics () {
  const counter = await Article.aggregate([
